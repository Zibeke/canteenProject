...
const getDashboardStats = async (req, res) => {
  try {
    ...
    res.json({
      ...
      pendingPenalties,
      closingTime:
        canteenConfig?.closingTime || "15:30",
    });
  } catch (error) {
...
const getCanteenConfig = async (req, res) => {
  try {
    ...
    let config = await CanteenConfig.findOne({ date: today });

    if (!config) {
      config = await CanteenConfig.create({
        date: today,
...
const updateCanteenConfig = async (req, res) => {
  try {
    ...
    let config = await CanteenConfig.findOneAndUpdate(
      { date: today },
      {
        closingTime,
